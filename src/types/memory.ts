...
export const memoryEntrySchema = z.object({
  ...
  metadata: z.object({
    ...
    lastUsed: z.string().refine((date) => !Number.isNaN(Date.parse(date)), {
      message: "Invalid ISO timestamp",
    }).optional(),
  }),
...
export const fillSessionSchema = z.object({
  ...
  status: z.enum(['detecting', 'matching', 'reviewing', 'filling', 'completed', 'failed']),
  startedAt: z.string().refine((date) => !Number.isNaN(Date.parse(date)), {
    message: "Invalid ISO timestamp",
  }),
  completedAt: z.string().refine((date) => !Number.isNaN(Date.parse(date)), {
    message: "Invalid ISO timestamp",
  }).optional(),
